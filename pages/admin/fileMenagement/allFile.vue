	...
	<div>
		<div class="w-full h-[90%]">
			<RichTable
				...
				<!-- ID列 -->
				<el-table-column label="ID" prop="id" width="120" align="center" show-overflow-tooltip>
				...
				<el-table-column label="预览" width="120" align="center">
					<template #default="{ row }">
						<el-image
							v-if="isImage(row.mimetype || row.type)"
							:src="row.url"
							:preview-src-list="[row.url]"
							fit="cover"
							class="w-16 h-16 cursor-pointer"
							preview-teleported />
						<el-button v-else type="primary" link size="small" @click="copyToClipboard(row.url)">
							复制链接
						</el-button>
					</template>
				</el-table-column>
				<!-- 原文件名列 -->
				<el-table-column label="原文件名" prop="originalName" min-width="200" show-overflow-tooltip>
					<template #default="{ row }">
				...
				<!-- MD5列 -->
				<el-table-column label="MD5" prop="md5" width="200" align="center" show-overflow-tooltip>
					<template #default="{ row }">
				...
				<!-- 文件大小列 -->
				<el-table-column label="文件大小" width="120" align="center" show-overflow-tooltip>
					<template #default="{ row }">
				...
				<!-- 操作列 -->
				<el-table-column label="操作" fixed="right" width="150" show-overflow-tooltip>
		...
		<div class="flex items-center justify-between py-2">
			<div class="mr-2 flex items-center">
				...
				<el-text type="info" size="small" class="ml-4"> 共 {{ total }} 个文件 </el-text>
			</div>
			<el-scrollbar>
				<ElsPagination
					:total="total"
...
const field: TableField[] = [
	{ type: 'text', label: '文件名', prop: 'name', visibled: false },
	{ type: 'text', label: '文件类型', prop: 'mimetype', visibled: false },
	{ type: 'text', label: '上传时间', prop: 'createdAt', visibled: false },
];
...
// 搜索（节流）
const t_handleSearch = useThrottle((keyword: string) => handleSearch(keyword), 1000);
